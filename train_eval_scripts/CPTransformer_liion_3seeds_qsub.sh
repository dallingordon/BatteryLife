...
#$ -pe omp 4
#$ -j y
#$ -o cptransformer_liion_3seeds.qlog
...
run_cptransformer () {
  local seed=$1
  local ckpt="/projectnb/nsf-energize/dgordon/Projects/BatteryLife/checkpoints/CPTransformer_MIXLARGE_seed${seed}"
  local log="${RESULTS_DIR}/CPTransformer_Liion_seed${seed}.log"
  mkdir -p "$ckpt"
  echo "=== CPTransformer | dataset=MIX_large seed=$seed ==="
  CUDA_VISIBLE_DEVICES=0 accelerate launch --num_processes 1 --main_process_port 25220 run_main.py \
    ...
    --train_epochs 100 --model_comment "CPTransformer_Liion_s${seed}" --accumulation_steps 1 \
...
grep -h "Best model performance" "$RESULTS_DIR"/CPTransformer_Liion_seed*.log
