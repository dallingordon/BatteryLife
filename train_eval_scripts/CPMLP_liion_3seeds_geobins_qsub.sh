...
#$ -pe omp 4
#$ -j y
#$ -o cpmlp_liion_3seeds_geobins.qlog
...
run_cpmlp () {
  local seed=$1
  local ckpt="/projectnb/nsf-energize/dgordon/Projects/BatteryLife/checkpoints/CPMLP_MIXLARGE_geobins_seed${seed}"
  local log="${RESULTS_DIR}/CPMLP_geobins_Liion_seed${seed}.log"
  mkdir -p "$ckpt"
  echo "=== CPMLP geo_bins | dataset=MIX_large seed=$seed ==="
  CUDA_VISIBLE_DEVICES=0 accelerate launch --num_processes 1 --main_process_port 20439 run_main.py \
    ...
    --train_epochs 100 --model_comment "CPMLP_geobins_Liion_s${seed}" --accumulation_steps 1 \
...
grep -h "Best model performance" "$RESULTS_DIR"/CPMLP_geobins_Liion_seed*.log
