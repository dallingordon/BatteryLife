#!/bin/bash -l
#$ -N cptransformer_naion_3seeds_geobins
#$ -P nsf-energize
#$ -l h_rt=01:30:00
#$ -l gpus=1
#$ -l gpu_c=6.0
#$ -pe omp 4
#$ -j y
#$ -o cptransformer_naion_3seeds_geobins.qlog
...
run_cptransformer () {
  ...
  local ckpt="/projectnb/nsf-energize/dgordon/Projects/BatteryLife/checkpoints/CPTransformer_NAION_geobins_seed${seed}"
  local log="${RESULTS_DIR}/CPTransformer_geobins_Naion_seed${seed}.log"
  mkdir -p "$ckpt"
  echo "=== CPTransformer geo_bins | dataset=$dataset seed=$seed ==="
  CUDA_VISIBLE_DEVICES=0 accelerate launch --num_processes 1 --main_process_port 25225 run_main.py \
    ...
    --train_epochs 100 --model_comment "CPTransformer_geobins_Naion_s${seed}" --accumulation_steps 1 \
...
grep -h "Best model performance" "$RESULTS_DIR"/CPTransformer_geobins_Naion_seed*.log
