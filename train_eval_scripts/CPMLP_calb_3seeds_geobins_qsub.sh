...
#$ -pe omp 4
#$ -j y
#$ -o cpmlp_calb_3seeds_geobins.qlog
...
run_cpmlp () {
  ...
  local ckpt="/projectnb/nsf-energize/dgordon/Projects/BatteryLife/checkpoints/CPMLP_CALB_geobins_seed${seed}"
  local log="${RESULTS_DIR}/CPMLP_geobins_${dataset}_seed${seed}.log"
  mkdir -p "$ckpt"
  echo "=== CPMLP geo_bins | dataset=$dataset seed=$seed bs=$batch_size d_model=$d_model d_ff=$d_ff e_layers=$e_layers d_layers=$d_layers dropout=$dropout lr=$lr ==="
  CUDA_VISIBLE_DEVICES=0 accelerate launch --num_processes 1 --main_process_port 20437 run_main.py \
    ...
    --train_epochs 100 --model_comment "CPMLP_geobins_${dataset}_s${seed}" --accumulation_steps 1 \
    --charge_discharge_length 300 --dataset "$dataset" --num_workers 4 \
    --e_layers "$e_layers" --lstm_layers 2 --d_layers "$d_layers" --patience 5 --n_heads 8 \
...
grep -h "Best model performance" "$RESULTS_DIR"/CPMLP_geobins_CALB*.log
