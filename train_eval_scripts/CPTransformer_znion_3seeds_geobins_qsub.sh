#!/bin/bash -l
#$ -N cptransformer_znion_3seeds_geobins
#$ -P nsf-energize
#$ -l h_rt=01:30:00
#$ -l gpus=1
#$ -l gpu_c=6.0
#$ -pe omp 4
#$ -j y
#$ -o cptransformer_znion_3seeds_geobins.qlog
...
run_cptransformer () {
  ...
  local ckpt="/projectnb/nsf-energize/dgordon/Projects/BatteryLife/checkpoints/CPTransformer_ZNION_geobins_seed${seed}"
  local log="${RESULTS_DIR}/CPTransformer_geobins_Znion_seed${seed}.log"
  mkdir -p "$ckpt"
  echo "=== CPTransformer geo_bins | dataset=$dataset seed=$seed ==="
  CUDA_VISIBLE_DEVICES=0 accelerate launch --num_processes 1 --main_process_port 25223 run_main.py \
    ...
    --d_model 64 --d_ff 128 --batch_size 64 --learning_rate 1e-03 \
    --train_epochs 100 --model_comment "CPTransformer_geobins_Znion_s${seed}" --accumulation_steps 1 \
    --charge_discharge_length 300 --dataset "$dataset" --num_workers 4 \
    --e_layers 1 --lstm_layers 6 --d_layers 11 --patience 5 --n_heads 4 \
...
run_cptransformer 2021 ZN-coin
run_cptransformer 42   ZN-coin42
run_cptransformer 2024 ZN-coin2024

echo "=== Best model performance lines across all 3 seeds (geo_bins) ==="
grep -h "Best model performance" "$RESULTS_DIR"/CPTransformer_geobins_Znion_seed*.log
