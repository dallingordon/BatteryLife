...
# ONE CPTransformer trained on all four chemistries pooled (Li-ion + CALB + Zn-ion + Na-ion).
...
run_pooled () {
  local seed=$1
  local ckpt="/projectnb/nsf-energize/dgordon/Projects/BatteryLife/checkpoints/CPTransformer_POOLED${TAG}_seed${seed}"
  local log="${RESULTS_DIR}/CPTransformer_Pooled${TAG}_seed${seed}.log"
  mkdir -p "$ckpt"
  echo "=== CPTransformer | POOLED${TAG} seed=$seed (split_seed=$seed) epochs=$EPOCHS ==="
  accelerate launch --num_processes 1 --main_process_port 25218 run_main.py \
    ...
    --train_epochs "$EPOCHS" --model_comment "CPTransformer_Pooled${TAG}_s${seed}" --accumulation_steps 1 \
...
grep -h "Pooled single-checkpoint\|Pooled per-chem-best-val\|=== Pooled per-chemistry" "$RESULTS_DIR"/CPTransformer_Pooled${TAG}_seed*.log
