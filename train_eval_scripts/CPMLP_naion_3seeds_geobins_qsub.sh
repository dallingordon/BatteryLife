...
#$ -pe omp 4
#$ -j y
#$ -o cpmlp_naion_3seeds_geobins.qlog
...
run_cpmlp () {
  ...
  local ckpt="/projectnb/nsf-energize/dgordon/Projects/BatteryLife/checkpoints/CPMLP_NAION_geobins_seed${seed}"
  local log="${RESULTS_DIR}/CPMLP_geobins_Naion_seed${seed}.log"
  mkdir -p "$ckpt"
  echo "=== CPMLP geo_bins | dataset=$dataset seed=$seed ==="
  CUDA_VISIBLE_DEVICES=0 accelerate launch --num_processes 1 --main_process_port 20443 run_main.py \
    ...
    --d_model 64 --d_ff 32 --batch_size 128 --learning_rate 5e-05 \
    --train_epochs 100 --model_comment "CPMLP_geobins_Naion_s${seed}" --accumulation_steps 1 \
    --charge_discharge_length 300 --dataset "$dataset" --num_workers 4 \
    --e_layers 1 --lstm_layers 2 --d_layers 9 --patience 5 --n_heads 8 \
...
run_cpmlp 2021 NAion
run_cpmlp 42   NAion42
run_cpmlp 2024 NAion2024

echo "=== Best model performance lines across all 3 seeds (geo_bins) ==="
grep -h "Best model performance" "$RESULTS_DIR"/CPMLP_geobins_Naion_seed*.log
