...
run_cpmlp () {
  ...
  local ckpt="/projectnb/nsf-energize/dgordon/Projects/BatteryLife/checkpoints/CPMLP_NAION_seed${seed}"
  local log="${RESULTS_DIR}/CPMLP_Naion_seed${seed}.log"
  mkdir -p "$ckpt"
  echo "=== CPMLP | dataset=$dataset seed=$seed ==="
  CUDA_VISIBLE_DEVICES=0 accelerate launch --num_processes 1 --main_process_port 20442 run_main.py \
    ...
    --train_epochs 100 --model_comment "CPMLP_Naion_s${seed}" --accumulation_steps 1 \
...
grep -h "Best model performance" "$RESULTS_DIR"/CPMLP_Naion_seed*.log
